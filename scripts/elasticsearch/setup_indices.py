...
def setup_UserIndex():
    ...
    # Create User index if not present
    if es.indices.exists(index=index_name_users):
        print("Index %s already exists. Skipping!"% index_name_users)
    else:
        res = es.indices.create(index=index_name_users, ignore=400, body=request_body_users)
        print("Result for Notification mapping")
        print(res)
